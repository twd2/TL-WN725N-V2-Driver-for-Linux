...
u32 rtw_start_drv_threads(_adapter *padapter)
{
	...
	if (!start_kthread(&padapter->xmitThread, rtw_xmit_thread, padapter,"rtl8188e-xmit"))
		_status = _FAIL;
#endif

#ifdef CONFIG_RECV_THREAD_MODE
	if(!start_kthread(&padapter->recvThread, rtw_recv_thread, padapter, "rtl8188e-recv"))
		_status = _FAIL;	
#endif

	if(!start_kthread(&padapter->cmdThread, rtw_cmd_thread, padapter, "rtl8188e-cmd"))
		_status = _FAIL;
	else
	...
	if(!start_kthread(&padapter->evtThread, event_thread, padapter, "rtl8188e-evt"))
		_status = _FAIL;		
#endif
